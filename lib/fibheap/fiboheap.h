...
public:
    ...
    public:
        FibNode(T k, V pl)
            : key(k), payload(pl), mark(false), p(nullptr), left(nullptr), right(nullptr), child(nullptr),
              degree(-1)
        {
        }
    ...
    void consolidate()
    {
        ...
        int d, rootSize;
        int max_degree = static_cast<int>(floor(log(static_cast<double>(n)) / log(static_cast<double>
                                                (1 + sqrt(static_cast<double>(5))) / 2)));

        // 1
        A = new FibNode*[max_degree +
                                    2]; // plus two both for indexing to max degree and so A[max_degree+1] == NIL
        // 2, 3
